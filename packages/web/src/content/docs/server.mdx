---
title: Server
description: Interact with costrict-cli server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `costrict-cli serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an costrict-cli client can use.
...
```bash
costrict-cli serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
costrict-cli serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `costrict-cli`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `costrict-cli serve` and `costrict-cli web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password costrict-cli serve
```
...
When you run `costrict-cli` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the costrict-cli server to interact with costrict-cli programmatically.
:::

This architecture lets costrict-cli support multiple clients and allows you to interact with costrict-cli programmatically.

You can run `costrict-cli serve` to start a standalone server. If you have the
costrict-cli TUI running, `costrict-cli serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CoStrict [IDE](/docs/ide) plugins.
...
The costrict-cli server exposes the following APIs.
