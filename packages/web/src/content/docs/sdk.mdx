---
title: SDK
description: Type-safe JS client for costrict-cli server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The costrict-cli JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control costrict-cli programmatically.
...
```bash
npm install @costrict-cli-ai/sdk
```
...
Create an instance of costrict-cli:

```javascript
import { createOpencode } from "@costrict-cli-ai/sdk"
...
const costrict-cli = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
If you already have a running instance of costrict-cli, you can create a client instance to connect to it:

```javascript
import { createOpencodeClient } from "@costrict-cli-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@costrict-cli-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*costrict-cli" },
})
