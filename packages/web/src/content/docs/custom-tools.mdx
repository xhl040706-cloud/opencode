...
Custom tools are functions you create that the LLM can call during conversations. They work alongside costrict-cli's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.costrict-cli/tool/` directory of your project.
- Or globally, by placing them in `~/.config/costrict-cli/tool/`.
...
```ts title=".costrict-cli/tool/database.ts" {1}
import { tool } from "@costrict-cli-ai/plugin"
...
```ts title=".costrict-cli/tool/math.ts"
import { tool } from "@costrict-cli-ai/plugin"
...
```ts title=".costrict-cli/tool/project.ts" {8}
import { tool } from "@costrict-cli-ai/plugin"
...
```python title=".costrict-cli/tool/add.py"
import sys
...
```ts title=".costrict-cli/tool/python-add.ts" {10}
import { tool } from "@costrict-cli-ai/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .costrict-cli/tool/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
