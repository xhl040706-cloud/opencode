...
With CoStrict you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, CoStrict uses our own `costrict-cli` theme.
...
CoStrict comes with several built-in themes.
...
- Want CoStrict to match their terminal's appearance
...
CoStrict supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/costrict-cli/themes/*.json` or `$XDG_CONFIG_HOME/costrict-cli/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.costrict-cli/themes/*.json`
4. **Current working directory** - Defined in `./.costrict-cli/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/costrict-cli/themes
vim ~/.config/costrict-cli/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .costrict-cli/themes
vim .costrict-cli/themes/my-theme.json
```
...
{
  "$schema": "https://costrict.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
