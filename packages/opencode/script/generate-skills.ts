...
const builtinEmbeddedTsFile = path.resolve(__dirname, "../.cache/builtin-skills-embedded.ts")
...
async function generateBuiltinSkills() {
  ...
  // Generate builtin.ts (dev mode: reads from filesystem, build mode: embedded content)
  await generateBuiltinTs()
  await generateBuiltinEmbeddedTs()
}
...
/**
 * Generate embedded version with all skill content baked in
 * This is used for compiled builds where filesystem access is not available
 */
async function generateBuiltinEmbeddedTs() {
  const skillNames = Object.keys(BUILTIN_SKILLS)

  // Ensure .cache directory exists
  await fs.mkdir(path.dirname(builtinEmbeddedTsFile), { recursive: true })
...
`
  ...
  await fs.writeFile(builtinEmbeddedTsFile, content, "utf-8")
  console.log(`✓ Generated ${builtinEmbeddedTsFile}`)
}
