...
export namespace Skill {
  ...
  export const state = Instance.state(async () => {
    const timer = log.time("startup.skill.state")
    ...
    try {
      ...
      {
        const step = log.time("startup.skill.initialize_builtin_skills")
        try {
      ...
      if (!Flag.OPENCODE_DISABLE_EXTERNAL_SKILLS) {
        const step = log.time("startup.skill.scan_external")
        try {
      ...
      {
        const step = log.time("startup.skill.scan_config_directories")
        try {
      ...
      {
        const step = log.time("startup.skill.scan_builtin_cache")
        try {
      ...
      {
        const step = log.time("startup.skill.scan_additional_paths")
        try {
      ...
      {
        const step = log.time("startup.skill.scan_urls")
        try {
          const config = await Config.get()
      ...
      {
        const step = log.time("startup.skill.scan_learning_candidates")
        try {
