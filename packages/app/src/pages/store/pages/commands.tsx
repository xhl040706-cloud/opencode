...
export default function Commands() {
  ...
  const [global, { refetch }] = createResource(
    ...
    ({ active }) => (active ? itemApi.list({ type: "command", limit: 500 }) : Promise.resolve(null)),
  )
        ...
        <div class="flex items-start justify-between gap-4">
          <div>
            <h1 class="text-2xl font-semibold mb-1 flex items-center gap-2.5 text-text-strong">
              <Icon name="console" class="text-green-500" /> {language.t("store.sidebar.nav.commands")}
            </h1>
            <p class="text-sm text-text-weak">{language.t("store.commands", { count: total() })}</p>
          </div>
          <Show when={user()}>
            <StoreCreateButton
              itemType="command"
