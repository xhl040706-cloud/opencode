...
export default function SecurityBadge(props: SecurityBadgeProps) {
   ...
   return (
      <Show when={props.status}>
         <span
            class={`inline-flex items-center gap-1 rounded font-medium ${size() === "sm" ? "px-1.5 py-0.5 text-xs" : "px-2.5 py-1 text-sm"
               } ${config().bgClass} ${config().textClass} ${config().animate ? "animate-pulse" : ""
               }`}
