...
const SheetOverlay = <T extends ValidComponent = "div">(
  ...
  return (
    <SheetPrimitive.Overlay
      class={cn(
        "fixed inset-0 z-50 bg-[rgba(0,0,0,0.72)] data-[expanded=]:animate-in data-[closed=]:animate-out data-[closed=]:fade-out-0 data-[expanded=]:fade-in-0",
        local.class
      )}
      {...others}
    />
...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-background-base p-6 shadow-[0_24px_64px_rgba(0,0,0,0.35)] transition ease-in-out data-[closed=]:duration-300 data-[expanded=]:duration-500 data-[expanded=]:animate-in data-[closed=]:animate-out",
  {
    variants: {
      position: {
...
const SheetContent = <T extends ValidComponent = "div">(
  ...
  return (
    <SheetPortal position={local.position}>
      ...
      >
        {local.children}
        <SheetPrimitive.CloseButton class="absolute right-4 top-4 z-20 cursor-pointer rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-secondary">
          <svg
