...
import { ChannelsSection } from "@/pages/store/components/channels-section"
...
type Tab = "repositories" | "capabilities" | "devices" | "notifications" | "channels"
...
export default function ConsolePage() {
  ...
  const [msgChannels, msgChannelActs] = createResource(async () => channelApi.list())
    ...
    <>
      <div class="console-page">
        ...
        <div class="console-stat-grid">
          ...
          <article
            class="store-stat-card"
            style={{ "--stat-accent": "#f59e0b", "--stat-bg": "#FEF3C7" }}
          ...
          >
            ...
            <div>
              <div class="store-stat-card-label">{language.t("console.stats.msgChannels")}</div>
              <p class="store-stat-card-value">
                <Show when={!msgChannels.loading} fallback="—">
                  {msgChannels()?.length ?? 0}
                </Show>
              </p>
            </div>
          </article>
          ...
          <div class="console-card-header">
            <div class="console-tabs" role="tablist">
              ...
              <button
                ...
                class={`console-tab ${activeTab() === "channels" ? "console-tab-active" : ""}`}
                onClick={() => setActiveTab("channels")}
              >
                {language.t("console.tab.channels")}
                <Show when={!msgChannels.loading}>
                  <span class="console-tab-badge">{msgChannels()?.length ?? 0}</span>
                </Show>
              </button>
            </div>
          </div>
          ...
          <Show when={activeTab() === "channels"}>
            <div class="console-tab-panel">
              <ChannelsSection
                ...
                loading={() => msgChannels.loading}
                setChannels={msgChannelActs.mutate}
              />
            </div>
          </Show>
        </section>
