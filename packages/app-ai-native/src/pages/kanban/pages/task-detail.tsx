            ...
            <section class="grid gap-4 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,0.9fr)]">
              <article class="rounded-[var(--native-radius-lg)] border border-[color:color-mix(in_oklab,var(--native-border)_24%,transparent)] bg-[var(--native-panel)] p-4 shadow-[var(--native-shadow-sm)]">
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div><div class="text-[11px] uppercase tracking-[0.12em] text-[var(--native-dim)]">{language.t("kanban.label.workDir")}</div><div class="mt-1 break-all text-sm text-[var(--native-foreground)]"><Show when={(task().work_dir_id || task().workDirId)?.trim()} fallback={text(task().work_dir)}><button type="button" class="text-left text-[var(--native-primary)] transition-colors hover:text-[var(--native-foreground)] cursor-pointer" onClick={() => { const wid = (task().work_dir_id || task().workDirId)!.trim(); navigate("/kanban/workdir/" + encodeURIComponent(wid) + "?fromTaskId=" + encodeURIComponent(task().task_id!.trim()) + (task().repo_addr ? "&workAddr=" + encodeURIComponent(task().repo_addr!.trim()) : "") + (task().repo_branch ? "&workBranch=" + encodeURIComponent(task().repo_branch!.trim()) : "")) }}>{task().work_dir || task().work_dir_id || task().workDirId}</button></Show></div></div>
