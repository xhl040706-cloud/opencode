...
function CommitManualDialog(props: { commit: CommitRow; onSaved?: () => void | Promise<void> }) {
  ...
  const submit = async (e: SubmitEvent) => {
    ...
    if (!commitId) {
      showToast({ variant: "error", title: language.t("kanban.toast.commitIdMissing") })
      return
    }
...
export default function KanbanCommitDetail() {
  ...
  const realExplain = createMemo(() => {
    ...
    if (!tasks().length) return language.t("kanban.validation.noTasks")
    return `Σ(Task actual time × silica) = ${tasks().map((item) => `${formatDuration(item.task_real_minutes, language.t)} × ${item.silica == null ? "0%" : `${(item.silica * 100).toFixed(1)}%`}`).join(" + ")}`
  })
      ...
      <div class="flex w-full flex-col gap-5">
        <header class="flex w-full flex-col gap-3">
          ...
          <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h1 class="m-0 font-[var(--native-font-display)] text-[1.875rem] leading-[1.02] font-semibold tracking-[-0.05em] text-[var(--native-foreground)]">{language.t("kanban.home.nav.commit")}</h1>
            </div>

            <div class="flex flex-wrap items-center gap-2">
              <Button size="sm" onClick={openManual} disabled={!commit().commit_id}>{language.t("kanban.dialog.manualAdjustment")}</Button>
            </div>
          </div>
        </header>

        <Show when={!data.loading} fallback={<div class="rounded-[var(--native-radius-lg)] border border-[color:color-mix(in_oklab,var(--native-border)_24%,transparent)] bg-[var(--native-panel)] px-4 py-10 text-sm text-[var(--native-muted)] shadow-[var(--native-shadow-sm)]">{language.t("kanban.loading.commitDetail")}</div>}>
          <Show when={data()} fallback={<div class="rounded-[var(--native-radius-lg)] border border-[color:color-mix(in_oklab,var(--native-border)_24%,transparent)] bg-[var(--native-panel)] px-4 py-10 text-sm text-[var(--native-muted)] shadow-[var(--native-shadow-sm)]">{language.t("kanban.empty.noCommitDetail")}</div>}>
            <section class="rounded-[var(--native-radius-lg)] border border-[color:color-mix(in_oklab,var(--native-border)_24%,transparent)] bg-[var(--native-panel)] p-4 shadow-[var(--native-shadow-sm)]">
              ...
              <div class="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div><div class="text-[11px] uppercase tracking-[0.12em] text-[var(--native-dim)]">{language.t("kanban.label.gitUser")}</div><div class="mt-1 break-all text-sm text-[var(--native-foreground)]">{text(commit().git_user_name ? `${commit().git_user_name} <${commit().git_user_email || ""}>` : "")}</div></div>
                <div>
                ...
                <div><div class="text-[11px] uppercase tracking-[0.12em] text-[var(--native-dim)]">{language.t("kanban.label.branch")}</div><div class="mt-1 text-sm text-[var(--native-foreground)]">{text(commit().repo_branch)}</div></div>
                <div><div class="text-[11px] uppercase tracking-[0.12em] text-[var(--native-dim)]">{language.t("kanban.label.commitTime")}</div><div class="mt-1 text-sm text-[var(--native-foreground)]">{formatLocalTime(commit().commit_time)}</div></div>
                <div class="md:col-span-2 xl:col-span-3"><div class="text-[11px] uppercase tracking-[0.12em] text-[var(--native-dim)]">{language.t("kanban.label.comment")}</div><div class="mt-1 text-sm text-[var(--native-foreground)]">{text(commit().comment)}</div></div>
              </div>
            </section>

            <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              <MetricCard label={language.t("kanban.metric.generatedCode")} value={commit().diff_lines == null ? "-" : `${commit().diff_lines} ${language.t("kanban.repo.lines")}`} accent="var(--native-warning)" />
              <MetricCard label={language.t("kanban.metric.actualTime")} value={formatDuration(commit().commit_real_minutes_manual ?? commit().commit_real_minutes, language.t)} hint={realExplain()} accent="var(--native-primary)" />
              <MetricCard label={language.t("kanban.metric.traditionalEst")} value={formatDuration(commit().commit_ancient_minutes_manual ?? commit().commit_ancient_minutes, language.t)} accent="var(--native-success)" />
              ...
              <MetricCard label={language.t("kanban.metric.cost")} value={fmtCost(totalCost())} accent="var(--native-warning)" />
              <article class="rounded-[var(--native-radius-lg)] border border-[color:color-mix(in_oklab,var(--native-border)_24%,transparent)] bg-[color:color-mix(in_oklab,var(--native-panel)_88%,var(--native-bg-subtle))] p-4 shadow-[var(--native-shadow-sm)]">
                <p class="m-0 text-[11px] font-semibold uppercase tracking-[0.12em] text-[color:color-mix(in_oklab,var(--native-success)_72%,var(--native-dim))]">{language.t("kanban.label.efficiencyRatioSilica")}</p>
                <div class="mt-3 flex flex-wrap items-center gap-3">
                  ...
                  <div class="inline-flex items-center gap-2 rounded-full border border-[color:color-mix(in_oklab,var(--native-success)_22%,transparent)] bg-[color:color-mix(in_oklab,var(--native-success)_10%,var(--native-panel))] px-3 py-1.5 text-sm font-medium text-[var(--native-foreground)]">
                    ...
                    <Tooltip value={language.t("kanban.hint.silicaExplanation")} placement="top">
            ...
            <section class="rounded-[var(--native-radius-lg)] border border-[color:color-mix(in_oklab,var(--native-border)_24%,transparent)] bg-[var(--native-panel)] shadow-[var(--native-shadow-sm)]">
              <div class="border-b border-[color:color-mix(in_oklab,var(--native-border)_24%,transparent)] px-4 py-3 text-[1rem] font-semibold text-[var(--native-foreground)]">{language.t("kanban.section.relatedTasks")}</div>
